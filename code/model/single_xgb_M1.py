...
if __name__ == '__main__':
    ...
    clf_M1= GridSearchCV(M1, parameters, scoring='roc_auc', n_jobs=1, cv=5)
    clf_M1.fit(train_X, train_y)
    for score in clf_M1.grid_scores_:
        print(score)
    print("Best score: ", clf_M1.best_score_)
    print("Best param: ", clf_M1.best_params_)

    # 预测、保存
    submit_M1 = test_data[['USRID']]
    t = test_data.drop(['USRID'], axis=1)[keep_feature]
    submit_M1['RST']  = clf_M1.predict_proba(t)[:, 1]
    submit_M1.to_csv('%s/submit_M1.csv' % output_dir, index=None,sep='\t')
